...
const __dirname = global.__dirname(
    ...
    (opts['mongodbv2'] ? new mongoDBV2(opts['db']) : new mongoDB(opts['db'])) : new JSONFile(`${opts._[0]?opts._[0]+'_':''}database.json`)) 
...
loadDatabase() 
global.authFile = `${opts._[0]||'yukino'}.data.json`
const {
    state,
    saveState
...
async function connectionUpdate(a) {
    ...
    if (a.receivedPendingNotifications) return this.sendButton(nomorown + '@s.whatsapp.net', 'Bot Successfully Connected', author, null, [
        ['MENU', '/menu']
    ], null)
}
...
global.reloadHandler = async function (a) {
    try {
...
_quickTest().then(() => conn.logger.info('Quick Test Done'))
.catch(console.error)
